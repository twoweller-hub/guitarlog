...
- 実装手順を示す際は、Claudeが行うコード変更だけでなく、ユーザーが行う作業（スプレッドシートの編集・GASの再デプロイ・動作確認など）も手順として明記する
...
- セッション中に保存した分だけカウント（ページをリロードするとリセット）
- スプレッドシートへのアクセス不要。`sessionSongs`・`sessionSeconds` 変数で管理
- 1曲も保存していない間は非表示
