...
一からこのアプリを再現するための手順書。開発中につまずいた点と解決策も記録している。最終更新: 2026-04-16（木）
...
| 関数 | 役割 |
|------|------|
| `doGet` | 曲リスト・通算回数・最終練習日を JSON で返す（JSONP 対応） |
| `doPost` | `action` フィールドで処理を分岐 |
| `logPractice` | 練習記録をシートに追記（通算回数は MAX+1 方式、メモはI列に保存） |
...
練習記録に任意のメモを残せるようにした。保存確認モーダルにテキスト入力欄（`#memo-input`）を追加し、入力内容をスプレッドシートのI列に保存する。「練習に戻る」を押してもメモの内容は消えない（保持する）。メモは空欄でも保存できる。GASの `logPractice()` で `sheet.getRange(newRow, 9).setValue(data.memo || '')` として書き込む。

---

### 19. 「今回の練習」サマリー表示を追加

練習タブの下部に、そのセッション中に保存した曲数と合計時間を表示する。スプレッドシートへのアクセスは不要で、`sessionSongs`・`sessionSeconds` のグローバル変数でカウントする。1曲も保存していない間は非表示（`display:none`）。ページをリロードするとリセットされる。複数端末で使っても問題ない設計。
